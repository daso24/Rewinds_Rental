...
public class InfoPelicula extends JFrame
{
    ...
    public InfoPelicula()
    {
        ...
        txtMonto = crearField(panelGris, "", 30, 305, 180);

        crearLabel(panelGris, "Descuento:", 30, 345);
        txtDescuento = crearField(panelGris, "", 30, 370, 180);
        ...
        lblImagenPelicula = new JLabel("", SwingConstants.CENTER);
        ...
        lblImagenPelicula.setForeground(Color.WHITE);

        lblImagenPelicula.addMouseListener(new MouseAdapter()
        {
            @Override
            public void mouseClicked(MouseEvent e)
            {
                if (enModoEdicion)
                {
                    ...
                    if (selector.showOpenDialog(null) == JFileChooser.APPROVE_OPTION)
                    {
                        ...
                        Image img = icon.getImage().getScaledInstance(lblImagenPelicula.getWidth(), lblImagenPelicula.getHeight(), Image.SCALE_SMOOTH);
                        lblImagenPelicula.setIcon(new ImageIcon(img));
                        lblImagenPelicula.setText("");
                    }
                }
            }
            ...
            public void mouseEntered(MouseEvent e)
            {
                if (enModoEdicion)
                {
                    lblImagenPelicula.setCursor(new Cursor(Cursor.HAND_CURSOR));
                    lblImagenPelicula.setBorder(new LineBorder(new Color(0, 170, 255), 3));
                }
            }
            ...
            public void mouseExited(MouseEvent e)
            {
                if (enModoEdicion)
                {
                    lblImagenPelicula.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
                    lblImagenPelicula.setBorder(new LineBorder(Color.WHITE, 2));
                }
            }
        });
        ...
        txtClasif = crearField(panelGris, "", 500, 355, 120);

        crearLabel(panelGris, "Año de lanzamiento:", 640, 335);
        txtAnio = crearField(panelGris, "", 640, 355, 120);

        crearLabel(panelGris, "Género:", 500, 395);
        txtGenero = crearField(panelGris, "", 500, 415, 260);
    ...
    public void setDatosPelicula(String nombre, String id, String tipo, String plataforma, String precioVenta, String descuento, String stockVenta, String stockRenta, String precioRenta, String clasificacion, String anio, String genero, ImageIcon caratula)
    {
        ...
        if (caratula != null)
        {
            ...
            this.lblImagenPelicula.setText("");
        }
        else
        {
            this.lblImagenPelicula.setIcon(null);
            this.lblImagenPelicula.setText("<html><center>Sin portada<br>disponible</center></html>");
        }
    }
    ...
    private JLabel crearLabelMenu(JPanel panel, String texto, String rutaIcono)
    {
        ...
        try
        {
            iconLabel.setIcon(new ImageIcon(new ImageIcon(getClass().getResource(rutaIcono)).getImage().getScaledInstance(35, 35, Image.SCALE_SMOOTH)));
        }
        catch(Exception e) {}
    ...
    public void cargarDatos(Object[] datos)
    {
        ...
        txtNomProd.setText(String.valueOf(datos[1]));
        txtTipo.setText("Película");
        txtFormato.setText(String.valueOf(datos[2]));
        txtMonto.setText("$" + datos[3]);
        txtFechaOp.setText("$" + datos[4]);
        txtStockVenta.setText(String.valueOf(datos[6]));
    ...
    public void cargarImagenPortada(String ruta)
    {
        lblImagenPelicula.setIcon(null);
        lblImagenPelicula.setText(""); 
        
        if (ruta == null || ruta.trim().isEmpty()) { 
            lblImagenPelicula.setText("Sin imagen"); 
            return; 
        }
        ...
        try {
            ...
            if (archivo.exists()) {
                ...
                URL urlImg = getClass().getResource(ruta.startsWith("/") ? ruta : "/" + ruta);
                if (urlImg != null) {
                    ImageIcon icon = new ImageIcon(urlImg);
                    Image img = icon.getImage().getScaledInstance(lblImagenPelicula.getWidth(), lblImagenPelicula.getHeight(), Image.SCALE_SMOOTH);
                    lblImagenPelicula.setIcon(new ImageIcon(img));
                } else {
                    lblImagenPelicula.setText("No encontrada");
                    System.out.println("DEBUG: No se encontró la ruta: " + ruta);
                }
            }
        } catch (Exception e) { 
            lblImagenPelicula.setText("Error"); 
        }
    }
